...
var general = module.exports = {
    log: function () {
        ...
        _.each(args, function (arg, i) {
            if (typeof arg === 'string') {
                ...
                args[i] = this.inspect(arg, {showHidden: true, colors: true, depth: 2});
            } else {
    ...
    getLogFn: function(showHidden, depth){
        return function () {
            var args = _.toArray(arguments);
            _.each(args, function (arg, i) {
                if (typeof arg === 'string') {
                    ...
                    args[i] = this.inspect(arg, {showHidden: showHidden, colors: true, depth: depth});
                } else {
                    args[i] = chalk.bold.cyan(' else: ' + typeof arg);
                }
            }.bind(this));
