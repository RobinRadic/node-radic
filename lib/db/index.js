...
types.file = function(DB){
    return new Datastore({ filename: DB._dbPath, autoload: true });
};
/**
...
function DB(name, options){
    ...
    this._dbPath = path.join(this.options.path, this.options.name + this.options.ext);
    this._db = types[this.options.type](this);
    this._models = {};
}
