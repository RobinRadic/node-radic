...
function assign( obj, methodName, c ){
    var assignToProperty = c.assignToProperty,
        fnCustomReturn = c.fnCustomReturn;
    return function(){
        obj[ assignToProperty ][ methodName ].apply(obj[ assignToProperty ], _.toArray(arguments));

        c._debug('return', assignToProperty, methodName, _.toArray(arguments))
        if( !_.isNull(fnCustomReturn) ){
            c._debug('custom return ', fnCustomReturn);
...
function eventer( obj, options ){
    ...
    var _log = util.getLogFn(false, 3);
    function debug(){
        if( c.debug !== false ){
            _log.apply(util, _.isString(c.debug) ? [cli.blue(c.debug) ].concat(_.toArray(arguments)) : _.toArray(arguments));
        }
    }
    c._debug = debug;
    ...
    if( c.assignByAliases ){
        ...
        _.each(c.aliases, function( aliases, methodName ){
            ...
            aliases.forEach(function( methodAssignmentName ){
                ...
                debug('assign', methodAssignmentName, methodName)
        ...
        [ 'assignMethods', 'assignPrivateMethods' ].forEach(function( methodType ){
            c[ methodType ].forEach(function( methodName ){
                ...
                c.aliases[ methodName ].forEach(function( methodAssignmentName ){
                    ...
                    debug('assign', methodName, '  to:', methodAssignmentName)
