...
module.exports = function(credentials, baseUrl) {

    return {
        get: function (url, data, cb, raw) {
            var command = "curl --GET --user " + credentials + " " + baseUrl + '/' + url;
            if(typeof data === 'object'){
                command += '?' + querystring.stringify(data);
            }/*
            if (cb) {
        ...
        post: function (url, data, cb, raw) {
            var command = "curl --user " + credentials + " " + baseUrl + '/' + url + " -H \"Content-Type: application/json\" --data '" + JSON.stringify(data) + "'";
            //console.log(command);
            run(command, raw, cb);
        },
        ...
        put: function (url, data, cb, raw) {
            ...
            var command = "curl --request PUT --user " + credentials + " " + baseUrl + '/' + url + " -H \"Content-Type: application/json\" --data '" + JSON.stringify(data) + "'";
            run(command, raw, cb);
        },

        del: function (url, cb, raw) {
            var command = "curl --request DELETE --user " + credentials + " " + baseUrl + '/' + url;
            run(command, raw, cb);
        }
    };
};
